...
import com.valentine.learnit.databinding.ActivityHealthAndFitnessBinding
...
class HealthAndFitnessActivity : AppCompatActivity() {
    private lateinit var binding: ActivityHealthAndFitnessBinding
    private val model: HealthAndFitnessViewModel by viewModels {
        HealthAndFitFactory((application as LearnItApplication).repository)
    }
    private val database = Firebase.database.reference
    private lateinit var firebaseAuth: FirebaseAuth

    private val adapters = HealthAndFitnessAdapter(ResultComparator)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityHealthAndFitnessBinding.inflate(layoutInflater)
        val view = binding.root
        setContentView(view)

        binding.toolbar5.apply {
            setSupportActionBar(this)
            supportActionBar?.setDisplayHomeAsUpEnabled(true)
            this.setNavigationOnClickListener {
                this@HealthAndFitnessActivity.onBackPressed()
            }
        }
        ...
        binding.healthFitList.apply {
            this.layoutManager = LinearLayoutManager(this@HealthAndFitnessActivity)
        ...
        lifecycleScope.launch {
            model.getHealthAndFitnessCourses().collectLatest {
        ...
        adapters.setOnItemClickListeners(object : OnItemClickedListener {
            override fun onItemClickedListener(result: Result) {
                val intent = Intent(this@HealthAndFitnessActivity, CourseDashboardActivity::class.java)
