...
import com.valentine.learnit.databinding.ActivityTeachingAndAccademicsBinding
...
class TeachingAndAcademicsActivity : AppCompatActivity() {
    private lateinit var binding: ActivityTeachingAndAccademicsBinding
    private val model: TeachingAndAcademicsViewModel by viewModels {
        TeachingAndAcademicsFactory((application as LearnItApplication).repository)
    }
    private val database = Firebase.database.reference
    private lateinit var firebaseAuth: FirebaseAuth

    private val adapters = TeachingAndAcademicsAdapter(ResultComparator)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityTeachingAndAccademicsBinding.inflate(layoutInflater)
        val view = binding.root
        setContentView(view)
        binding.toolbar11.apply {
            setSupportActionBar(this)
            supportActionBar?.setDisplayHomeAsUpEnabled(true)
            this.setNavigationOnClickListener {
                this@TeachingAndAcademicsActivity.onBackPressed()
            }
        }
        ...
        binding.teachingAcadeList.apply {
            this.layoutManager = LinearLayoutManager(this@TeachingAndAcademicsActivity)
        ...
        lifecycleScope.launch {
            model.getTeachingAndAcademicsCourses().collectLatest {
        ...
        adapters.setItemClickListener(object : OnItemClickedListener {
            override fun onItemClickedListener(result: Result) {
                val intent = Intent(this@TeachingAndAcademicsActivity, CourseDashboardActivity::class.java)
